...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads=8, d_model=D_MODEL):
        ...
        # attention layers
        self.attention_layers = [Attention(d_model=D_MODEL, d_k=self.head_dim) for _ in range(num_heads)] 
    ...
    def forward(self, x):
        ...
        W_q = self.W_query_layer(x)
        W_k = self.W_key_layer(x)
        W_v = self.W_value_layer(x)
        
        print("shape: ", W_q.shape)
